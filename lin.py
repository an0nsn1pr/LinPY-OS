...
print("Finding the secrets that goverments hide")

secrets = ["Jews run the world", "CIA is a criminal organisation", "9/11 was an inside job", "Jews control the banks",
"America and Israel created ISIS", "Jews made the first economy inflation", "Spain had their GOLDEN time when they expeled jews",
"Its imposibile to cook 6 milion pizzas in 5 years, best i can do is 271 thousand", "The media is controlled by the jews", 
"The Adult industry is mostly runned by the jews(even the gay type)", "JFK got killed because he didnt support Israel", 
" Sanhedrin 59a: \"Murdering Goyim is like killing a wild animal.\"", ]

print("These dont look good but loaded them anyway     (100%)")
...
def nano_editor(stdscr, editfilename):
    ...
    y, x = 0, 0  # Cursor position in the file (not screen)
    ...
    while True:
        ...
        # 🔹 Draw file content starting from line 5
        for idx, line in enumerate(lines):
            stdscr.addstr(idx + 5, 0, line)

        stdscr.move(y + 5, x)  # Offset by 5 lines due to header
        stdscr.refresh()
...
print("Got the directory                               (100%)")
print("Changing Directory to ROOT FOLDER")
os.chdir(filesystempath)
cdirpath = filesystempath
...
if setup["setupcheck"] == False:
    ...
    while i > 0:
        ...
        passlogin = input("Password:  ").encode()
        h.update(passlogin)

        if setup["passw"] == h.hexdigest():
...
{clear}     -       clears screen
{secret}    -       tells a secret that the goverments dont want you to know
...
{pingcmd}      -       {ping} [IP or Domain] [optional: a number that will ping the IP x4]
{timecmd}      -       {timecmd}
